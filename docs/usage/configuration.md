...
- Any other file of your choice with the `--config` CLI argument
...
```bash
$ semantic-release --config path/to/config.json
```
...
Type: `String`<br>
CLI arguments: `-c`, `--config`

Specify the path of the configuration file to use. The filename can be customized.

Using this option disables the automatic search for configuration files.

**Note**: When using `--config` with a file named `package.json`, the configuration must be under the `release` property.
