...
import { extractGitLogTags, extractGitLogNotes } from "./utils.js";
...
export async function getTagsNotes(execaOptions) {
  /**
   * git log --tags="*" --decorate-refs="refs/tags/*" --no-walk --format="%d%x09%N%x00" --notes="refs/notes/semantic-release*"
   *
   *  (tag: v1.2.3)
  ...
  const { stdout } = await execa(
    ...
    [
      ...
      "--format=%d%x09%N%x00", // <refName><tab><notes> eg. (tag: v24.2.3)  {"channels":[null]}
  ...
  // drop empty lines
  const lines = stdout.split("\0").filter((line) => line.trim() !== "");
  ...
  for (const line of lines) {
    ...
    try {
      const notes = extractGitLogNotes(notePart);
      tags.forEach((tag) => {
        tagNotesMap.set(tag, notes);
      });
    } catch (error) {
      debug(error);
    }
