...
export async function getTagsNotes(execaOptions) {
  ...
  // Track the last parsed tags to handle continuation lines.
  // When multiple notes refs (e.g., refs/notes/semantic-release-v1.0.0 and
  // refs/notes/semantic-release-v2.0.0) exist for the same commit, git
  // concatenates notes with newlines in %N, producing extra lines without
  // tag decorations that must be merged with the preceding entry.
  let lastTags = [];
  ...
  for (const line of lines) {
    ...
    } else if (lastTags.length > 0 && line.trim()) {
      // Continuation line: a note from an additional refs/notes/semantic-release-*
      // ref on the same commit. Merge its channels with the existing entry.
      try {
        const parsed = JSON.parse(line.trim());
        if (parsed.channels) {
          for (const tag of lastTags) {
            const existing = tagNotesMap.get(tag);
            if (existing?.channels) {
              const merged = new Set([...existing.channels, ...parsed.channels]);
