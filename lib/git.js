 ...
 * @param {Boolean} annotateTag True when the tag shall be annotated, false otherwise.
 * @param {String} ref The Git reference to tag.
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 * @throws {Error} if the tag creation failed.
 */
async function tag(tagName, annotateTag, ref, execaOpts) {
  const taggingFlags = ['tag', tagName];
  if (annotateTag !== undefined && annotateTag !== null && annotateTag) {
    taggingFlags.push('-a');
    taggingFlags.push('-m');
    taggingFlags.push(tagName);
  }
  taggingFlags.push(ref);

  await execa('git', taggingFlags, execaOptions);
}
