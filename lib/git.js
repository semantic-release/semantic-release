...
export async function isBranchUpToDate(repositoryUrl, branch, execaOptions) {
  ...
  try {
    const { exitCode } = await execa("git", ["merge-base", "--is-ancestor", localHead, remoteHead], {
      ...execaOptions,
      reject: false,
    });

    /**
     * Exit code 0 means we are BEHIND i.e. local is ancestor of remote - return false
     * Any other exit code means we're either AHEAD or DIVERGED - return true
     */
    return exitCode !== 0;
  } catch (error) {
    debug(error);
