 ...
 * Get all the tags for a given branch.
 *
 * @param {String} glob The glob the tags should match
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 * @return {Array<String>} List of git tags.
 * @throws {Error} If the `git` command fails.
 */
async function getTagsByGlob(glob, execaOptions) {
  return (await execa('git', ['tag', '--list', glob], execaOptions)).stdout
    .split('\n')
    .map((tag) => tag.trim())
    .filter(Boolean);
}
