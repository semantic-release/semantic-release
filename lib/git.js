...
/**
 * Get latest tag for a given branch.
 *
...
export async function getTags(branch, execaOptions) {
  try {
    ...
    debug("getTags error: %s", error.stderr);
    if (error.exitCode === 128 && error.stderr.includes("No names found, cannot describe anything.")) {
