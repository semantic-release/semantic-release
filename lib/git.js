...
export async function addNote(note, ref, execaOptions) {
  const notesRef = `refs/notes/${GIT_NOTE_REF}-${ref}`;
  const temporaryRef = `refs/notes/${GIT_NOTE_REF}-tmp-${randomUUID()}`;
  const previous = await execa("git", ["rev-parse", "--verify", "--quiet", notesRef], execaOptions).then(
    ({ stdout }) => stdout,
    (error) => {
      if (error.exitCode === 1) return undefined;
      throw error;
    }
  );
  ...
  try {
    if (previous) {
      await execa("git", ["update-ref", temporaryRef, previous, ""], execaOptions);
    }
    // Let Git maintain the notes tree, but keep its non-conventional commit off the published ref.
    await execa("git", ["notes", "--ref", temporaryRef, "add", "-f", "-m", JSON.stringify(note), ref], execaOptions);
    const { stdout: tree } = await execa("git", ["rev-parse", `${temporaryRef}^{tree}`], execaOptions);
    if (previous) {
      const { stdout: previousTree } = await execa("git", ["rev-parse", `${previous}^{tree}`], execaOptions);
      if (tree === previousTree) return;
    }
    const { stdout: commit } = await execa(
      ...
      [
        ...
        ...(previous ? ["-p", previous] : []),
        "-m",
        `chore(release): add channel notes for ${ref}`,
      ],
      execaOptions
    );
    // Preserve published history and fail if another writer changed the notes ref meanwhile.
    await execa("git", ["update-ref", notesRef, commit, previous ?? ""], execaOptions);
  } finally {
    await execa("git", ["update-ref", "-d", temporaryRef], execaOptions);
  }
}
