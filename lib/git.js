...
/**
 * Verify the write access authorization to remote repository with tags push dry-run.
 *
 * @param {String} repositoryUrl The remote repository URL.
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 ...
 */
export async function verifyAuth(repositoryUrl, execaOptions) {
  try {
    await execa("git", ["push", "--dry-run", "--no-verify", "--", repositoryUrl], execaOptions);

    const tags = (await execa("git", ["tag", "--list"], execaOptions)).stdout
      .split("\n")
      .map((tag) => tag.trim())
      .filter(Boolean);
    ...
    if (/tip of your current branch is behind|rejected|non-fast-forward/i.test(message)) {
      return;
    }
 ...
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 * @return {Boolean} `true` only if the local branch is ahead of the remote branch, falsy otherwise.
 */
export async function isBranchUpToDate(repositoryUrl, branch, execaOptions) {
  const localHead = await getGitHead(execaOptions);
  const { stdout } = await execa("git", ["ls-remote", "--heads", "--", repositoryUrl, branch], execaOptions);
  const remoteHead = stdout.match(/^(?<ref>\w+)/)?.[0];

  if (!remoteHead) return false;
  if (localHead === remoteHead) return true;

  try {
    const { exitCode } = await execa("git", ["merge-base", "--is-ancestor", "--", remoteHead, localHead], {
      ...execaOptions,
      reject: false,
    });

    /**
     * Exit code 0 means remote is BEHIND local (local is AHEAD) - return true
     * Any other exit code means local is either BEHIND or DIVERGED of remote - return false
     */
    return exitCode === 0;
  } catch (error) {
    debug(error);
