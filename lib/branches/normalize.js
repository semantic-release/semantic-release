...
function maintenance({maintenance, release, prerelease}) {
  return sortBy(
    ...
    const maintenanceMin =
      ...
      isMajorRange(range) && branches[idx - 1] && !isMajorRange(branches[idx - 1].range)
        ...
        : // Otherwise the lowest bound is the lowest bound of the current branch range
          getLowerBound(range);
    const maintenanceMax =
      // If the current branch has a major range (1.x or 1.x.x) and the previous doesn't
      isMajorRange(range) && branches[idx - 1] && !isMajorRange(branches[idx - 1].range)
        ? // Then the lowest bound is the upper bound of the previous branch range
          getUpperBound(branches[idx - 1].range)
        : // Otherwise the upper bound is the upper bound of the current branch range
          getUpperBound(range);
    ...
    // Get only the prerelease versions that are in the range of the prerelease branch
    const prereleaseVersionsInRange = tagsToVersions(prerelease?.[0] ? prerelease[0].tags : []).filter((v) =>
      semver.lt(v, maintenanceMax)
    );
    const releaseVersions = tagsToVersions(release?.[0] ? release[0].tags : []);
    // Get the latest possible version for the release branch
    const latestReleaseVersion = getLatestVersion(releaseVersions);
    // Get the latest possible version for the prerelease branch
    const latestPrereleaseVersion = getLatestVersion(prereleaseVersionsInRange, {withPrerelease: true});

    // The latest version will be the highest version between the latest release and prerelease version
    const latestVersion = highest(latestPrereleaseVersion, latestReleaseVersion);

    // Determine the first release of the default branch or any prerelease branch not present in any maintenance branch
    const base = (release[0] && (getFirstVersion(releaseVersions, branches) || latestVersion)) || FIRST_RELEASE;
