...
module.exports = async ({ repositoryUrl, branch }) => {
  ...
  if (!await verifyAuth(repositoryUrl, branch)) {
    ...
    return gitCredentials
      ? (repositoryUrl.includes('visualstudio.com'))
        ? format({ ...parse(`${gitUrlParse(repositoryUrl).toString(protocol)}`), ...{ auth: gitCredentials } }) : format({ ...parse(`${gitUrlParse(repositoryUrl).toString(protocol)}.git`), ...{ auth: gitCredentials } })
      : repositoryUrl;
  }
