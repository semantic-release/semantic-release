...
Please make sure to add the \`repositoryUrl\` to the [semantic-release configuration] (${linkify(
  ...
  EGITNOPERMISSION: ({options}) => ({
    message: 'The push permission to the Git repository is missing or repository is not available by given repositoryUrl.',
...
Also, please check, that repository is available via ${options.repositoryUrl}. If it is offline or does not exist, that would cause this error as well.`,
  }),
