...
Your configuration for the problematic branches is \`${stringify(branches)}\`.`,
  ...
  ENORELEASEBRANCHES: ({options: {repositoryUrl}, branch: {name}}) => ({
    message: 'Branch does not exist on remote',
    details: `The default release branch (${name}) does not exist on remote ${repositoryUrl}. Create the branch and try again.`,
  }),
